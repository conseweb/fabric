...
PWD := $(shell pwd)
REMOTE_HOST := 138.197.4.41
push:
	rsync -vaz --delete --exclude=.git --exclude=peer --exclude=bin $(PWD) root@$(REMOTE_HOST):/opt/
...
run: 
	docker run --rm -it \
	 --name dev \
	 --net host \
	 -v $(PWD):/opt/gopath/src/github.com/hyperledger/fabric \
	 -v /var/lib/docker.sock:/var/lib/docker.sock \
	 -w /opt/gopath/src/github.com/hyperledger/fabric \
	 ckeyer/obc:dev bash

deploy:
	./obc-peer chaincode deploy -p github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example02 -c '{"Function":"init", "Args": ["a","100", "b", "200"]}'

CC_NAME := a

invoke:
	./obc-peer chaincode invoke -p github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example02 -n $(CC_NAME) -c '{"Function":"init", "Args": ["a","100", "b", "200"]}'
